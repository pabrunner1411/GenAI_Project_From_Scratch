...
def get_embedding_model(model_name: str) -> SentenceTransformer:
    """Lädt ein Embedding-Modell einmalig und cached es danach."""
...
def hard_split_by_tokens(
    ...
    """Fallback für einzelne Einheiten, die zu lang sind."""
...
def split_atomic_units(
    ...
    """
    Bildet kleine Grundeinheiten.

    Kurze Absätze bleiben erhalten. Lange Absätze werden in Satzgruppen
    zerlegt. Extrem lange Sätze werden tokenbasiert geteilt.
    """
    paragraphs = [
        re.sub(r"[ \t\n]+", " ", paragraph).strip()
...
def semantic_chunk_text(
    ...
    """
    Teilt Text an semantischen Themenwechseln.

    Niedrige Ähnlichkeit zwischen zwei benachbarten Einheiten gilt als
    mögliche Chunkgrenze. min_tokens und max_tokens begrenzen die Größe.
    """
    if min_tokens <= 0:
        raise ValueError("Die minimale Chunkgröße muss größer als 0 sein.")

    if max_tokens <= min_tokens:
        raise ValueError(
            "Die maximale Chunkgröße muss größer als die minimale sein."
        )

    if not 0 <= breakpoint_percentile <= 100:
        raise ValueError(
            "Der Trennstellen-Prozentwert muss zwischen 0 und 100 liegen."
        )

    if overlap_units < 0:
        raise ValueError("Der Overlap darf nicht negativ sein.")
