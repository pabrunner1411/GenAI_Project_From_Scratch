...
def get_embedding_model(model_name: str) -> SentenceTransformer:
    """Loads an embedding model once and caches it afterwards."""
...
def hard_split_by_tokens(
    ...
    """Fallback for individual units that are too long."""
...
def split_atomic_units(
    ...
    """
    Forms small base units.

    Short paragraphs are kept as-is. Long paragraphs are split into
    sentence groups. Extremely long sentences are split token-wise.
    """
    paragraphs = [
        re.sub(r"[ \t\n]+", " ", paragraph).strip()
...
def semantic_chunk_text(
    ...
    """
    Splits text at semantic topic shifts.

    Low similarity between two neighboring units counts as a
    possible chunk boundary. min_tokens and max_tokens bound the size.
    """
    if min_tokens <= 0:
    ...
    if overlap_units < 0:
        raise ValueError("The overlap must not be negative.")
