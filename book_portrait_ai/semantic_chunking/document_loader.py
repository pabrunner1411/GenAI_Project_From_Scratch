...
def read_plain_text(path: Path) -> str:
    """Reads TXT and Markdown files with several encoding fallbacks."""
...
def read_pdf(path: Path) -> str:
    """Extracts the existing text layer from a PDF file."""
...
def extract_text(path: Path) -> str:
    """Selects the matching loader based on the file extension."""
    suffix = path.suffix.lower()

    if suffix not in SUPPORTED_SUFFIXES:
        allowed = ", ".join(sorted(SUPPORTED_SUFFIXES))
        raise ValueError(
            f"Unsupported file format: {suffix}. Allowed: {allowed}"
        )
